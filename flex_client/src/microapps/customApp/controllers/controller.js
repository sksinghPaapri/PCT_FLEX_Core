...
const AppController = {
    ...
    initForm(reset) {
        ...
        return ApiService.get(`customDocumentType/customDocument/${MOUNT.CURRENT_RECORD_ID}?docType=${MOUNT.DOCUMENT_TYPE_ID}`)
            .then(response => {
                if (response.data.isSuccess) {
                    MOUNT.CURRENT_DOCUMENT = response.data.document;
    ...
    updateNetSuite() {
        ...
        MOUNT.ERP_UPDATING(true);
        ApiService.setHeader();
        return ApiService.patch(`/netsuite/${MOUNT.DOCUMENT_TYPE_ID}`, MOUNT.CURRENT_DOCUMENT)
            .then(response => {
                console.log(response)
                MOUNT.ERP_UPDATING(false);
                if (response.data.isSuccess)
                    MOUNT.NAVIGATE(`/${MOUNT.ROOT_PATH}/customapp/list?doctype=${MOUNT.DOCUMENT_TYPE_ID}`)
            })
            .catch(error => {
