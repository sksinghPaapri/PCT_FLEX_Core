...
export default function JobPositionList() {
    ...
    const findAllDocument = async () => {
        ...
        const response = await ApiService.get('jobPosition');
    ...
    const columns = [
        {
            headerName: ' ', field: 'id', sortable: false, filter: false, cellRenderer: (params) =>
                <>
                    <Button style={{ minWidth: "4rem" }} size="sm" as={Link} to={`/${rootPath}/jobpositions/edit/${params.value}`}><BsBoxArrowInUpRight /></Button>
                    {/* <Button style={{ minWidth: "4rem" }} size="sm" as={Link} to={`/employees/employee/${params.value}?mode=view`}><BsEyeFill /></Button> */}
                </>
        },
        { headerName: 'NAME', field: 'name' },
        { headerName: 'DESCRIPTION', field: 'jobDescription' },
        // { headerName: 'Number of Person', field: 'numberOfPerson' },
                ...
                <Container fluid >
                    <Row>
                        <Col className='p-0 ps-2'>
                            <Breadcrumb style={{ fontSize: '24px', marginBottom: '0 !important' }}>
                                <Breadcrumb.Item active> <div className='breadcrum-label-active'>JOB POSITIONS</div></Breadcrumb.Item>
                    ...
                    <Row style={{ marginTop: '-10px' }}>
                        <Col className='p-0 ps-1'>
                            <Button size="sm" as={Link} to={`/${rootPath}/jobpositions/add`}>CREATE</Button>{" "}
                        </Col>
                        <Col md="4" sm="6">
                            <Row>
